...
class StraightPath(Path):
    ...
    def __init__(self, canvas, start_x, start_y, end_x, end_y, width, color="black"):
    ...
    def create_path(self):
        ...
        if self.path_length > 0:
            ...
            # 創建多邊形路徑
            self.path_rect = self.canvas.create_polygon(
                points,
                fill=self.color,
                outline=self.color
            )
            ...
            self.path_rect = self.canvas.create_oval(
    ...
    def _calculate_path_points(self):
        ...
        return [
            current_start_x + perp_x, current_start_y + perp_y,  # 左上
            self.end_x + perp_x, self.end_y + perp_y,            # 右上
            self.end_x - perp_x, self.end_y - perp_y,            # 右下
            current_start_x - perp_x, current_start_y - perp_y   # 左下
        ]
    ...
    def is_inside(self, x, y):
        ...
        # 計算當前終點位置
        ratio = self.current_length / self.path_length
        current_end_x = self.start_x + self.dx * ratio
        current_end_y = self.start_y + self.dy * ratio
        
        # 計算投影參數
        current_dx = current_end_x - self.start_x
        current_dy = current_end_y - self.start_y
        ...
        # 計算最近點
        nearest_x = self.start_x + t * current_dx
        nearest_y = self.start_y + t * current_dy
    ...
    def shrink(self):
        ...
        if self.current_length > 0:
            self.current_length = max(0, self.current_length - self.shrink_speed)
            ...
            if self.path_length > 0:
                ...
                # 圓形路徑的處理
                half_width = max(0, self.width // 2 * (self.current_length / self.path_length))
                self.canvas.coords(
                    self.path_rect,
                    self.start_x - half_width, self.start_y - half_width,
                    self.start_x + half_width, self.start_y + half_width
...
class CornerPath(Path):
    ...
    def __init__(self, canvas, start_x, start_y, corner_x, corner_y, end_x, end_y, width, color="black"):
        super().__init__(canvas, width, color)
        self.start_x = start_x
        self.start_y = start_y
        ...
        # 計算路徑段
        self.segment1_length = math.sqrt((corner_x - start_x)**2 + (corner_y - start_y)**2)
        self.segment2_length = math.sqrt((end_x - corner_x)**2 + (end_y - corner_y)**2)
    ...
    def create_path(self):
        """創建轉彎路徑"""
        # 創建第一段（起點到轉彎點）
        self.segment1 = self._create_segment(
            self.start_x, self.start_y, 
            self.corner_x + (self.width / 2), self.corner_y
        )

        # 創建第二段（轉彎點到終點）
        self.segment2 = self._create_segment(
    ...
    def _create_segment(self, x1, y1, x2, y2):
        ...
        # 創建多邊形
        points = [
            x1 + perp_x, y1 + perp_y,  # 左上
            x2 + perp_x, y2 + perp_y,  # 右上
            x2 - perp_x, y2 - perp_y,  # 右下
            x1 - perp_x, y1 - perp_y   # 左下
        ]

        return self.canvas.create_polygon(points, fill=self.color, outline=self.color)
    
    def is_inside(self, x, y):
        """檢查點是否在轉彎路徑內"""
        # 檢查是否在第一段路徑內
        if self._point_in_segment(x, y, self.start_x, self.start_y, self.corner_x, self.corner_y):
            return True
        
        # 檢查是否在第二段路徑內
        if self._point_in_segment(x, y, self.corner_x, self.corner_y, self.end_x, self.end_y):
            return True
    ...
    def shrink(self):
        ...
        if self.current_progress > 0:
            self.current_progress = max(0, self.current_progress - self.shrink_speed / self.total_length)
            self._update_path()
    ...
    def get_goal_area(self):
        ...
        if length == 0:
            return {'left': self.end_x, 'top': self.end_y, 'right': self.end_x, 'bottom': self.end_y}
...
class PathFollowingTestApp:
    ...
    def setup_goal(self):
        ...
        if 'points' in goal_area:
            ...
            self.goal_rect = self.canvas.create_rectangle(
                goal_area['left'], goal_area['top'],
                goal_area['right'], goal_area['bottom'],
                fill=self.goal_color
            )
    ...
    def check_reached_goal(self):
        ...
        if 'points' in goal_area:
            ...
            return (min_x <= self.player_x <= max_x and 
                    min_y <= self.player_y <= max_y)
        else:
            # 矩形目標區域的檢查
            return (goal_area['left'] <= self.player_x <= goal_area['right'] and
...
if __name__ == "__main__":
    ...
    listener = ControllerInput(
        analog_callback=app.on_joycon_input
    )
